...
class NetInterface {
 ...
 public:
  ...
  void setcost(int v){          // getter and setter of the cost
    this->cost = v;
  }
...
class Node {
 ...
 public:
  ...
  void addInterface(string ip, string connip, int cost, Node *nextHop) {
    ...
    eth.setcost(cost);                      // set the cost the interface
  ...
  void sendMsg(){
    ...
    for (int i = 0; i < interfaces.size(); ++i) {
      ...
      msg.cost = interfaces[i].first.getcost();   // get the cost of the edge/interface through which we are sending this message
